...
!BOP
!
! !ROUTINE: gdasnc_reset_interp_input
!  \label{gdasnc_reset_interp_input}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine gdasnc_reset_interp_input(n, findex, gridDesci)
! !USES:
...
!EOP
   ...
   if ( LIS_isatAfinerResolution(n,gridDesci(9)) ) then
      ...
      write(LIS_logunit,*) '[INFO] The GDAS forcing resolution is coarser ' // &
      ...
      write(LIS_logunit,*) '[INFO] The GDAS forcing resolution is finer ' // &
                           'than the running domain.'
      ...
      select case( gdas_struc(n)%met_interp )
       ...
       case default
         ...
         write(LIS_logunit,*) gdas_struc(n)%met_interp,", "
         write(LIS_logunit,*) 'is not supported for GDAS.'
         call LIS_endrun()
      end select
   endif

end subroutine gdasnc_reset_interp_input
