...
!
! !INTERFACE:
subroutine get_gdasnc(n, findex)
! !USES:
...
!
!  See additional notes in create\_gdasnc\_filename(\ref{create_gdasnc_filename}).
!
!  The arguments are: 
!  \begin{description}
!  \item[n]
...
!  \item[create\_gdasnc\_filename](\ref{create_gdas_filename}) \newline
...
!EOP
  ...
  real    :: gmt1, gmt2
  character(len=120) :: name00, name03, name06
...
!= T126:
  if ( time1 >= gdas_struc(n)%griduptime1 .and. &
     ...
     gridDesci(10) = 95
     gridDesci(20) = 0
     gdas_struc(n)%gridDesci(:) = gridDesci(:)

     if(LIS_rc%met_ecor(findex).eq."lapse-rate" .or. &
          LIS_rc%met_ecor(findex) .eq. "lapse-rate and slope-aspect") then 
        call read_gdasnc_elev(n,findex, 1)
     endif

     call gdasnc_reset_interp_input(n, findex, gridDesci)
...
!= T170:
  elseif ( time1 >= gdas_struc(n)%griduptime2 .and. &
     ...
     if(LIS_rc%met_ecor(findex).eq."lapse-rate" .or. &
          LIS_rc%met_ecor(findex) .eq. "lapse-rate and slope-aspect") then 
        call read_gdasnc_elev(n,findex, 2)
     endif
...
!= T254:
  elseif ( time1 >= gdas_struc(n)%griduptime3 .and. & 
     ...
     write(LIS_logunit,*) '[INFO] GDAS -- changing grid to 2002 - 2005 (T254)'
     ...
     if(LIS_rc%met_ecor(findex).eq."lapse-rate" .or. &
          LIS_rc%met_ecor(findex) .eq. "lapse-rate and slope-aspect") then 
        call read_gdasnc_elev(n,findex, 3)
     endif
...
!= T382:
  elseif ( time1 >= gdas_struc(n)%griduptime4 .and. &
     ...
     if(LIS_rc%met_ecor(findex).eq."lapse-rate" .or. &
          LIS_rc%met_ecor(findex) .eq. "lapse-rate and slope-aspect") then 
        call read_gdasnc_elev(n,findex, 4)
     endif
...
!= T574:
  elseif ( time1 >= gdas_struc(n)%griduptime5 .and. &
     ...
     if(LIS_rc%met_ecor(findex).eq."lapse-rate" .or. &
          LIS_rc%met_ecor(findex) .eq. "lapse-rate and slope-aspect") then 
        call read_gdasnc_elev(n,findex, 5)
     endif
...
!= T1534:
  elseif ( time1 >= gdas_struc(n)%griduptime6 .and. &
     ...
     if(LIS_rc%met_ecor(findex).eq."lapse-rate" .or. &
          LIS_rc%met_ecor(findex) .eq. "lapse-rate and slope-aspect") then 
        call read_gdasnc_elev(n,findex, 6)
     endif
  ...
  if ( gdas_struc(n)%findtime1 == 1 ) then  !get new time1 from the past
     ...
     do
        ...
        call create_gdasnc_filename(order, name00, name03, name06, F06flag, &
        ...
        if ( .not. file_exists1 ) then
           status = 1
           write(LIS_logunit,*) '[WARN] GDAS file1 (I) ',trim(name00), &
                'does not exist. (f00) (try = ', try, ')'
        endif
        if ( .not. file_exists2 ) then
           status = 1
           write(LIS_logunit,*) '[WARN] GDAS file1 (A) ',trim(name03), &
        ...
        if ( ( .not. file_exists1 ) .or. ( .not. file_exists2 ) ) then 
           ...
           call create_gdasncf9_filename(order, name00, name03, &
           ...
           if ( status == 1 ) then
              ...
              if ( .not. file_exists1 ) then
                 status = 1
                 write(LIS_logunit,*) '[WARN] backup GDAS f09 file1 (I) ', &
                      trim(name00), &
                      'does not exist. (f00) (try = ',try,')'
              endif
              if ( .not. file_exists2 ) then
                 status = 1
                 write(LIS_logunit,*) '[WARN] backup GDAS f09 file1 (A) ', &
                      trim(name03), &
                      'does not exist. (f03) (try = ',try,')'
              endif
        ...
        if ( F06flag ) then
           ...
           if ( .not. file_exists3 ) then
              status = 1
              write(LIS_logunit,*) '[WARN] GDAS file1 (A) ',trim(name06), &
        ...
        if ( status == 0 ) then 
           ...
           call read_gdasnc(order, n, findex,  &
           ...
           if ( ferror == 1 ) then  
              ...
              write(LIS_logunit,*) '[WARN] Error reading files. ', &
  ...
  if ( gdas_struc(n)%findtime2 == 1 ) then  
     ...
     do
        ...
        if ( .not. file_exists2 ) then
           status = 1
           write(LIS_logunit,*) '[WARN] GDAS file2 (A) ',trim(name03), &
                'does not exist. (f03) (try = ', try, ')'
        endif
        ...
        if ( ( .not. file_exists1 ) .or. ( .not. file_exists2 ) ) then
           ...
           if ( status == 1 ) then
              ...
              if ( .not. file_exists1 ) then
                 status = 1
                 write(LIS_logunit,*) '[WARN] GDAS backup f09 file2 (I) ', &
                      trim(name00), &
                      'does not exist. (f00) (try = ',try,')'
              endif
              if ( .not. file_exists2 ) then
                 status = 1
                 write(LIS_logunit,*) '[WARN] GDAS backup f09 file2 (A) ', &
                      trim(name03), &
                      'does not exist. (f03) (try = ',try,')'
              endif
        ...
        if ( F06flag ) then
           ...
           if ( .not. file_exists3 ) then
              status = 1
              write(LIS_logunit,*) '[WARN] GDAS file2 (A) ',trim(name06), &
                   'does not exist. (f06) (try = ', try, ')'
           endif
        endif
