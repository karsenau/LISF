...
!BOP
! !ROUTINE: create_gdasnc_filename
! \label{create_gdasncfilename}
!
! !INTERFACE:
subroutine create_gdasnc_filename(option, name00, name03, name06, &
...
!-----------------------------------------------------------------
  ...
  write(unit=fsubs, fmt='(a16, a2, a4)') '.gdas1.sfluxgrbf', fcstcode0, '.nc4'
  ...
  write(unit=fdir, fmt='(a1, i4.4, i2.2, a1)') '/', uyr0, umo0, '/'
  write(unit=ftime, fmt='(i4.4, i2.2, i2.2, a2)') uyr0, umo0, uda0, analysis_hour_avg
  write(unit=fsubs, fmt='(a16, a2, a4)') '.gdas1.sfluxgrbf', fcstcode2, '.nc4'
  name06 = trim(gdasdir)//fdir//ftime//fsubs

end subroutine create_gdasnc_filename
