...
!BOP
! !ROUTINE: create_gdasncf9_filename
! \label{create_gdasncf9_filename}
!
! !INTERFACE:
subroutine create_gdasncf9_filename(option, name00, name03, &
...
!
! !DESCRIPTION:
!   This subroutine puts together 9hr forecast GDAS file names
!
!   First read the notes in
!   create\_gdasnc\_filename.F90 (\ref{create_gdasnc_filename}).
!
!   This routine is used to create the GDAS file name for the backup 9hr 
!   forecast file.  At analysis hours (hr = 00, 06, 12, and 18), LIS
...
!
!   For example, when LIS is at hour 06, it reads ahead to get instantaneous
...
!   Should a name00 or name03 file be missing when LIS is at a non-analysis
...
!
!   Note that the case where LIS is reading name00, name03, and name06
!   data to read ahead is not handled by this routine.
!   Example, consider the case when reading ahead to hour 18.
!   Here LIS is at hour 15, and LIS needs 18f00 for instantaneous values
!   at hour 18, and LIS needs 12f03 and 12f06 for averaged values for the
!   period [15, 18].  12f03 contains averaged values for [12, 15].  If it
!   is missing, LIS could use 06f09 to obtain averaged values for [12, 15].
!   Again, this scenario is not handled by this routine.  LIS will simply
...
!-----------------------------------------------------------------
  ...
  if ( option == 2 ) then !bookend 2
     ...
     if ( uhr == 0 .or. uhr == 6 .or. uhr == 12 .or. uhr == 18 ) then
        ...
        write(initcode1,'(i2.2)') uhr-6

        !special case: need to go back to the previous day 18z
        if ( uhr0 == 0 ) then
           initcode1 = '18'
           call LIS_tick(dumbtime, doy, gmt, uyr0, umo0, uda0, &
  ...
  if ( status == 0 ) then
     ...
     write(UNIT=fsubs, fmt='(a16, a2, a4)') '.gdas1.sfluxgrbf', fcstcode1, '.nc4'
...
end subroutine create_gdasncf9_filename
